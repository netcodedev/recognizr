...
async fn recognize_handler(
    ...
    for face in &mut faces {
        ...
        let mut response = state.db
            .query("SELECT name, vector::similarity::cosine(embedding, $query) AS distance FROM person ORDER BY distance ASC LIMIT 1")
            .bind(("query", embedding))
            .await?;

        if let Some(mut db_res) = response.take::<Option<RecognitionResult>>(0)? {
            // Now, set the bbox value before adding it to our final results.
...
async fn debug_detector_handler(
    ...
    // Encode the modified image back to bytes (e.g., PNG format)
